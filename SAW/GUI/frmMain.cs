	...
	public sealed partial class frmMain : KeyForm, IParameterConsumer, IKeyControl
	{
		...
		public frmMain()
		{
			try
			{
				...
				Globals.VerbApplicabilityChanged += Engine_VerbApplicabilityChanged;
				...
				Functions.MouseStep.SetMouseStep(AppliedConfig.MouseSteps.Medium);
		...
		public void Display()
		{
			...
			Palette.Item(Config.ToolboxPaletteID).Position.Dock = DockStyle.None;
			...
			Palette.Item(Config.ToolboxPaletteID).Position.Bounds.Size = new Size(150, 300); // note A:R will be adjusted later to match definition, keeping diagonal length the same, so X or Y could change if A:R wrong here
			Show();
			RebuildDocking();
			m_Displayed = true;
			MatchWindowToSet();
			Palette.Item("DocumentOutline").Position.MoveBesideForm(this, false);
			Palette.Item(Config.ToolboxPaletteID).Position.MoveBesideForm(this, true);
			RebuildDocking();
		}
		...
		{
			//Debug.WriteLine("View resized.  Screen bounds=" + Bounds + ", view=" + pnlView.Bounds);
		}
		...
		private void BuildUserPalettes()
		{
			...
			Palette.Register(new Palette(Parameters.LineColour, pnlLineColour, "[Palette_LineColour]", "LineColour"), ttMain);
			...
			Palette.Register(new Palette(Parameters.FillColour, pnlFillColour, "[Palette_FillColour]", "FillColour"), ttMain);
			...
			Palette.Register(new Palette(Parameters.FontSize, pnlTextStyle, "[Palette_TextStyle]", "TextStyle"), ttMain);
		}
		...
		private void BuildAdvancedPalettes()
		{
			...
			Palette.Register(new Palette(Parameters.ArrowheadEndType, m_pnlArrowheads, "[Palette_Arrowheads]", "Arrowheads"), ttMain);
			...
			Palette.Register(new Palette("DocumentOutline", Palette.Purpose.Specials.DocumentOutline, m_pnlDocumentOutline, "[Document_Outline]", "DocumentOutline_24"), ttMain);
		...
		private void ApplyConfiguration() //(Optional eChangeUser As Users = -1)
		{
			...
			try
			{
				...
				int size = Globals.Root.CurrentConfig.ReadInteger(Config.Toolbar_ButtonSize, Config.Toolbar_ButtonSize_Default);
				if (size != m_CurrentTBSize)
					ResizeToolbar(size); // Slowish(?) so only done if needed
				ApplyConfigurationToMiscControls(shapes);
				ApplyConfigurationVisibility();
				Palette.UpdateCustomPalettes(Globals.Root.CurrentConfig, ttMain);
				if (userChanged)
					Config.UserCurrent.RestorePalettePositions(); // must be after UpdateCustomPalettes but before RebuildDocking
				RebuildDocking();
				CreateToolbarButtons();
				m_CurrentTBSize = size;
				pnlToolbar.ResumeLayout();
				this.ResumeLayout();
				//For Each ctr As Control In Me.Controls
				//	AM.Log.WriteLine(ctr.GetType.ToString + "/" + ctr.Name + " @ " + ctr.Bounds.ToString + IIf(ctr.Visible, "", " not visible"))
				//Next

				// reset various things
				Shape.EnvironmentChanges eChange = Shape.EnvironmentChanges.Settings;
				if (userChanged)
					eChange = eChange | Shape.EnvironmentChanges.User;
				m_Document?.NotifySettingsChanged(eChange);
				...
				SetWindowTitle();
				Engine_VerbApplicabilityChanged();

				if (oldLowGraphics != Globals.Root.CurrentConfig.Low_Graphics)
				{
					pnlToolbar.Refresh();
					pnlLeft.Refresh();
					foreach (Palette objPalette in Palette.List.Values)
					{
						objPalette.Control.Refresh();
					}
				}
		...
		/// <summary>designed at default size.  Standard sizes are 40, 48, 80.  These are the actual buttons; the panel is about new relate bigger and the images within the buttons 16 smaller</summary>
		private int m_CurrentTBSize = 40;

		private void ResizeToolbar(int size)
		{
			// only called if changed
			Size radioSize = new Size(size + 8, size + 8);


			pnlSnap.SuspendLayout();
			rdoSnapAngle.Size = radioSize;
			rdoSnapGrid.Size = radioSize;
			rdoSnapShape.Size = radioSize;
			pnlSnap.Width = radioSize.Width * 3 + 17; // paddings etc don't change.  Is +17 in designer at normal size
			pnlSnap.Height = radioSize.Height;
			// mustnt set m_CurrentTBSize as caller wants to be able to detect changes yet
			pnlSnap.ResumeLayout();
		}
		...
		private void CreateToolbarButtons()
		{
			...
			int size = Globals.Root.CurrentConfig.ReadInteger(Config.Toolbar_ButtonSize, Config.Toolbar_ButtonSize_Default); // button size
			bool sizeChanged = size != m_CurrentTBSize;
			...
			for (int index = 0; index <= verbs.Count - 1; index++)
			{
				...
				if (index < m_ToolbarButtons.Count)
				{
					...
					if (sizeChanged)
					{
						button.MinimumSize = new Size(sizeAdjusted, sizeAdjusted);
		...
		private void SetToolImageSizes()
		{
			...
			try
			{
				...
				const int step = 16;
				switch (Globals.Root.CurrentConfig.ReadInteger(Config.Toolbar_ButtonSize)) // numbers below are image sizes: + indicates add excess(16)
				{
					case 40: // is default(128).  Allows 2*48+
						break;
					case 48:
						permittedWidth = (64 + SharedButton.EXCESSSIZE) * 2; // 160 allows 2*64+.  3*48+ would need 196 - quite a bit bigger
						break;
					case 80:
						permittedWidth = 240; // 2cols at 96+ or 80+, 3 at 64+;  228 would have allows 2 at 96+ but still only 2 at 64+ 196 + RoundButton.EXCESSSIZE * 2
						size = 96 + SharedButton.EXCESSSIZE;
						break;
				}
		...
		public void DisplayPage(int index, bool documentChanged = false)
		{
			...
			try
			{
				...
				Engine_VerbApplicabilityChanged(); // can include page buttons

				DisplayPage_Application(index, documentChanged);
		...
		private void DisplayPage_Application(int index, bool documentChanged)
		{
		}
		...
		internal void IndirectEffectsUndoRedo(Transaction transaction, bool isUndo)
		{
			...
			else
			{
				...
				transaction.CurrentPage?.UpdateAllIntersections();
				btnPageNext.Enabled = m_PageIndex < m_Document.Count - 1 || !m_Page.IsEmpty;
			}
			if (transaction.ContainsConfig)
				Globals.OnSettingsChanged();
		...
		private void Engine_VerbApplicabilityChanged() // VerbButtonsUpdateEnabled
		{
			...
			foreach (Palette palette in from p in m_DisplayedPalettes.Values where p.Control is PaletteView select p)
			{
				((PaletteView)palette.Control).UpdateVerbApplicability();
			}
		}
		...
		private void CreateContextMenus() // called by constructor
		{
			...
			// Add one item from the options menu:
			//m_mnuContext.Items.Add(New ToolStripMenuItem With {.Text = mnuAddToPalette.Text, .Tag = mnuAddToPalette.Tag, .Image = mnuAddToPalette.Image})
			// doesn't work since InitialiseMenu will ignore the tag, and not add a handler.
		...
		private void SetDefaultStyles(bool startup)
		{
			...
			if (!startup)
			{
				...
				{
					if (document?.InitialLineStyle != null) // They must either all be nothing or all defined.  This also checks if objDocument is nothing
					{
		...
		{
			//var d = new Desktop();
			//d.PopulateFromWindows();
			//d.Save(@"d:\temp\desktop.dsk");
			//SAW.GUI.frmEditDesktop.Display(d);

			var d = Desktop.LoadFrom(@"d:\temp\desktop.dsk");
			d.OpenApplications();
		}
