...
namespace SAW.Functions
{
	...
	public abstract class Action
	{
		...
		public override string ToString() => Convert.ToInt32(Change) + "=" + PersistValueText;
		...
		public virtual Shape.Prompt GetPrompt() => null;
	...
	internal class ToolAction : ValueSelectableAction
	{
		...
		public override void Trigger(EditableView.ClickPosition.Sources source, EditableView pnlView, Transaction transaction)
		{
			if (Globals.Root.CurrentConfig.ReadBoolean(Config.Use_Speech))
				Globals.Root.Speech.Speak(DescriptionWithoutAccelerator());
	...
	public class PaletteAction : Action
	{
		...
		public override string PersistValueText
		{
			...
			{
				if (Purpose.IsCustom)
					return Purpose.CustomID.ToString();
				return Purpose.ToInt32().ToString();
			}
		}
		...
		public PaletteAction(string persist) : base(Parameters.Action_Palette)
		{
			...
			//if (Information.IsNumeric(action.TextValue))
			//	TriggerPalette(new Palette.Purpose(action.ValueAsInteger), true);
			//else
			//	TriggerPalette(new Palette.Purpose(new Guid(action.TextValue)), true);
		}
		#endregion
	...
	internal class PaletteButtonAction : Action
	{
		...
		public override string DescriptionWithAccelerator(bool forSpeech = false)
		{
			switch (Index)
			{
				...
				case 3: return Strings.Item("Config_Keys_Palette_Rainbow");
				default:
