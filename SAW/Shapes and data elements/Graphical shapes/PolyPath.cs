...
namespace SAW
{
	...
	public abstract class PolyPath : Filled
	{
		...
		// The user cannot draw this, it is only generated by combining other shapes. The data is stored in a graphics path.  Base class for IndependentPolyPath and Splatter
		...
		// Targets only in IndependentPolyPath.  GrabSpots in this class, but Splatter can override them to block them again
		...
		public override void ApplyTransformation(Transformation transformation)
		{
			...
			m_Path.Transform(transformation.Matrix); // Lined does not transform the path, it just tries to discard it
		...
		{
			CheckIntersectionsWithSelfUsingPath(EnsurePath(true));  // Will be exterior path if IsWide
		}
	...
	public class IndependentPolyPath : PolyPath
	{
		// This has no need to keep links to the original shapes - it can't anyway, because the original shapes are deleted

		public IndependentPolyPath() // version for serialisation etc
		{ }

		public IndependentPolyPath(GraphicsPath path)
		{
			m_Path = path;
		...
		public override Shapes ShapeCode => Shapes.IndependentPolyPath;
		...
		protected override void DoGrabMove(GrabMovement move)
		{
			...
			switch (move.GrabType)
			{
				case GrabTypes.Bezier:
					...
					var vertex = -1; // the index of the vertex; only defined if intOther is defined
		...
		public override bool VertexVerbApplicable(Functions.Codes code, Target target)
		{
			...
			switch (code)
			{
				...
				case Functions.Codes.CornerVertex:
					...
					var straight = Math.Abs(sizeA.CrossProduct(sizeB)) < Geometry.NEGLIGIBLEANGLE;
					return (code == Functions.Codes.SmoothVertex) ^ straight;
				case Functions.Codes.ConvertToBezier:
					var element1 = PathElement.GetElement(m_Path, target.ShapeIndex);
					return element1.IsLine;
				case Functions.Codes.ConvertToLine:
					var element = PathElement.GetElement(m_Path, target.ShapeIndex);
		...
		{
			Target target = ((EditableView)position.View).PathTarget;
			switch (code)
			{
				case Functions.Codes.RemoveVertex:
