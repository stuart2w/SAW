...
InstallDir "$PROGRAMFILES\SAW"

; Registry key to check for directory (so if you install again, it will 
; overwrite the old one automatically)
InstallDirRegKey HKLM "Software\SAW" "Install_Dir"
...
NetCheckDone:
  ...
  WriteRegStr HKLM "Software\SAW" InstallSource $EXEDIR
  ...
  CreateDirectory "$SMPROGRAMS\SAW"
  CreateShortCut "$SMPROGRAMS\SAW\SAW 7.lnk" "$INSTDIR\SAW.exe" "" "$INSTDIR\SAW.exe" 0

  CreateShortCut "$DESKTOP\SAW 7.lnk" "$INSTDIR\SAW.exe" ""
  
  ; Write the installation path into the registry
  WriteRegStr HKLM SOFTWARE\SAW "Install_Dir" "$INSTDIR"
  
  ; Write the uninstall keys for Windows
  WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\SAW" "DisplayName" "SAW"
  WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\SAW" "UninstallString" '"$INSTDIR\uninstall.exe"'
  WriteRegDWORD HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\SAW" "NoModify" 1
  WriteRegDWORD HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\SAW" "NoRepair" 1
...
Section "Uninstall"
  ...
  DeleteRegKey HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\SAW"

  ; Remove files and uninstaller
  Delete $INSTDIR\uninstall.exe

  ; Remove shortcuts, if any
  Delete "$SMPROGRAMS\SAW\*.*"
  RMDir "$SMPROGRAMS\SAW"
